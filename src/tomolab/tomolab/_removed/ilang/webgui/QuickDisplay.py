...
def display_image(image, background=False, new_tab=True):
    ...
    setup_local_files()
    if new_tab:
        open_new('http://%s:%s/ilang_viewport.html' % (address,str(port)))
    run_webserver(background)


def image_ipython_notebook(image):
    display_image(image, background=False, new_tab=False)
    return '<iframe src=http://%s:%s/ilang_viewport.html width=800 height=500 frameborder=0></iframe>' % (address,str(port))
...
def display_graph(graph, background=False, new_tab=True):
    ...
    fid.close()
    if new_tab:
        open_new('http://%s:%s/ilang_graph.html' % (address,str(port)))
    run_webserver(background)


def graph_ipython_notebook(graph, background=True):
    display_graph(graph, background=background, new_tab=False)
    url = 'http://%s:%s/ilang_graph.html' % (address,str(port))
    print(url)
