...
from tomolab.global_settings import has_svgwrite
import numpy as np
...
class Michelogram:
    ...
    def _make(self):
        ...
        n_planes = np.zeros([n_sinograms_per_side + 1], dtype=np.int32)
        ...
        self.michelogram_sinogram = -1 * np.ones([self.n_rings, self.n_rings], order="F", dtype=np.int32)
        self.michelogram_plane = -1 * np.ones([self.n_rings, self.n_rings], order="F", dtype=np.int32)

        # -2- right sinograms
        sinogram_index = 0
        for X in range(0, self.max_ring_diff, span):
            if sinogram_index == 0:
    ...
    def _make_svg(self):
        if not has_svgwrite:
            ...
            dwg = svgwrite.Drawing(
                "michelogram.svg", size=(w, h), profile="full", debug=True
            )
