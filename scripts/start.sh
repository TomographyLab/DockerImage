...
if [ $(id -u) == 0 ] ; then
    ...
    # Only attempt to change the occiput username if it exists
    if id occiput &> /dev/null ; then
        echo "Set username to: $NB_USER"
        usermod -d /home/$NB_USER -l $NB_USER occiput
    fi
    ...
    if [[ "$NB_USER" != "occiput" ]]; then
        # changing username, make sure homedir exists
        # (it could be mounted, and we shouldn't create it if it already exists)
        if [[ ! -e "/home/$NB_USER" ]]; then
            echo "Relocating home dir to /home/$NB_USER"
            mv /home/occiput "/home/$NB_USER"
        fi
        # if workdir is in /home/occiput, cd to /home/$NB_USER
        if [[ "$PWD/" == "/home/occiput/"* ]]; then
    ...
    if [[ "$NB_UID" == "$(id -u occiput)" && "$NB_GID" == "$(id -g occiput)" ]]; then
        ...
        if [[ "$STATUS" != "0" ]]; then
            if [[ -w /etc/passwd ]]; then
                ...
                cat /etc/passwd | sed -e "s/^occiput:/nayvoj:/" > /tmp/passwd
                echo "occiput:x:$(id -u):$(id -g):,,,:/home/occiput:/bin/bash" >> /tmp/passwd
